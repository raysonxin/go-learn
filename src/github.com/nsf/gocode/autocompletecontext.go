...
func (c *auto_complete_context) get_candidates_from_decl(cc cursor_context, class decl_class, b *out_buffers) {
	...
	for _, decl := range cc.decl.children {
		...
		if b != nil && b.candidates != nil {
			fmt.Println("BBB:    " + decl.name)
		}
	...
	if b != nil && b.candidates != nil {
		fmt.Println("CCC:    " + strconv.Itoa(len(b.candidates)))
	}
...
func fixup_packages(filescope *scope, pkgs []package_import, pcache package_cache) {
	for _, p := range pkgs {
		...
		if pcache[path].main != nil && pcache[path].main.children != nil {
			fmt.Println(("replace1    " + alias + "    Number   " + strconv.Itoa(len(pcache[path].main.children))))
		}
